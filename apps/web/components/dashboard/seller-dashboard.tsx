...
import { ShoppingBag, Package, Brain, ArrowUpRight, TrendingUp, AlertTriangle, Users } from 'lucide-react'
...
export default function SellerDashboard({ data }: { data: any }) {
  ...
  const recentSales = Array.isArray(transactions)
    ? transactions.filter((tx: any) => tx.type === 'REVENUE').slice(0, 6)
    : []

  const kpis = [
    {
      label: t('totalRevenue'),
      value: `$${(cash?.totalIncome || 0).toLocaleString()}`,
      sub: `Net: $${(cash?.netCashFlow || 0).toLocaleString()}`,
      color: 'text-success',
    ...
    },
    {
      label: t('unitsSold'),
      value: totalSold,
      sub: `${productList.length} ${t('product')}`,
      color: 'text-blue-600',
    ...
    },
    {
      ...
      sub: lowStock.length > 0 ? t('needsRestock') : t('allGood'),
      color: lowStock.length > 0 ? 'text-danger' : 'text-success',
      border: lowStock.length > 0 ? 'border-l-red-500' : 'border-l-green-500',
  ...
  return (
    <div className="space-y-8 p-4 lg:p-8">
      <div className="flex justify-between items-end">
        <div>
          <p className="text-xs font-black text-accent2 uppercase tracking-widest mb-1">{t('sellerDashboardTitle')}</p>
          <h1 className="text-3xl font-black font-outfit text-text tracking-tight">{t('myStoreOverview')}</h1>
        </div>
        <div className="hidden sm:flex items-center gap-2 bg-white px-4 py-2 rounded-2xl border border-text/5 shadow-sm">
          <div className="w-2 h-2 bg-success rounded-full animate-pulse" />
          <span className="text-xs font-black text-text/40 uppercase tracking-widest">{t('liveLabel')}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map(k => (
          <div key={k.label} className={`bg-white dark:bg-secondary/10 rounded-2xl border-l-4 ${k.border} p-5 shadow-sm`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Recent Sales */}
        <div className="lg:col-span-2 bg-white dark:bg-secondary/10 rounded-3xl border border-text/5 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-text/5 flex justify-between items-center">
            <div>
              <h3 className="font-black font-outfit uppercase tracking-tight">{t('recentSales')}</h3>
              <p className="text-[10px] text-text/40 uppercase tracking-widest mt-0.5">{t('latestRevenueTx')}</p>
            </div>
            <Link href="/dashboard/finances" className="text-xs font-black text-accent2 hover:underline">{t('viewAll')}</Link>
          </div>
          {recentSales.length === 0 ? (
            <div className="p-8 text-center text-text/30">
              <ShoppingBag className="w-10 h-10 mx-auto mb-3 opacity-30" />
              <p className="font-bold text-sm">{t('noSalesYet')}</p>
            </div>
          ) : (
        ...
        <div className="space-y-4">
          <Link href="/dashboard/inventory" className="block bg-white dark:bg-secondary/10 rounded-3xl border border-text/5 shadow-sm p-6 hover:shadow-lg transition-all group">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-black font-outfit uppercase tracking-tight text-sm">{t('inventory')}</h3>
            ...
            <p className="text-[10px] text-text/40 uppercase tracking-widest mt-1">{t('totalProducts')}</p>
            {lowStock.length > 0 && (
              <div className="mt-4 p-3 bg-red-50 rounded-xl">
                <p className="text-xs font-black text-red-600">{lowStock.length} {t('itemsLowStock')}</p>
          ...
          <Link href="/dashboard/ai-insights" className="block bg-gradient-to-br from-text to-text/90 text-white rounded-3xl p-6 shadow-2xl hover:scale-[1.02] transition-all group">
            <div className="flex items-center gap-2 mb-3 opacity-60">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest">{t('aiAdvice')}</span>
            </div>
            <p className="text-sm font-bold leading-snug group-hover:text-accent2 transition-colors line-clamp-3">
              {aiInsight ? `"${aiInsight.description}"` : `"${t('aiAdviceEmpty')}"`}
            </p>
      ...
      {productList.length > 0 && (
        <div className="bg-white dark:bg-secondary/10 rounded-3xl border border-text/5 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-text/5 flex justify-between items-center">
            <h3 className="font-black font-outfit uppercase tracking-tight">{t('topProducts')}</h3>
            <Link href="/dashboard/inventory" className="text-xs font-black text-accent2 hover:underline">{t('manageLabel')}</Link>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-secondary/30 text-text/40 text-[10px] uppercase tracking-widest">
                <tr>
                  <th className="px-6 py-3 font-black">{t('product')}</th>
                  <th className="px-6 py-3 font-black">{t('stock')}</th>
                  <th className="px-6 py-3 font-black">{t('sold')}</th>
                  <th className="px-6 py-3 font-black">{t('revenue')}</th>
                  <th className="px-6 py-3 font-black">{t('status')}</th>
                </tr>
              </thead>
