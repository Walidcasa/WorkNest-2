...
export function AddClientModal({ isOpen, onClose, onSuccess }: AddClientModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await dashboardApi.addClient(form)
      onSuccess()
      onClose()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center p-6 border-b border-text/5">
          <h2 className="text-xl font-bold font-outfit uppercase tracking-tight">{t('newClient')}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="block text-xs font-bold text-text/50 mb-2 uppercase tracking-widest">{t('emailAddress')}</label>
              <input
                type="email"
                required
                value={form.email}
          ...
          <div className="pt-4">
            ...
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : t('addClient')}
            </button>
          </div>
        </form>
      </div>
